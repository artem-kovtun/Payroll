  ...
  ""CurrencyList"" : {
    ...
    ""PER10000"" : [
      ...
      },
      {
        ...
        {
            this.currency = currency ?? throw new ArgumentNullException("currency is null");
            this.language = language ?? throw new ArgumentNullException("language is null");
            this.pennies = pennies ?? throw new ArgumentNullException("pennies is null");
            string languageElement = language;
            dynamic currencyList = JsonConvert.DeserializeObject(json);
            ...
            foreach (var languageItem in items)
            {
                ...
                {
                    var texts = (string)languageItem["-text"];
                    var key = (string)languageItem["-value"];
                    messages[key] = texts.Split(',');
                }
            }
        ...
        public Money ConvertValueExtended(double money)
        {
            long intPart = (long)money;
            long fractPart = (long)Math.Round((money - intPart) * NUM100);
            if (currency == "PER1000")
            {
                fractPart = (long)Math.Round((money - intPart) * NUM1000);
            }
            var value = Convert(intPart, fractPart);
            var firstPart = value.Substring(0, value.IndexOf(fractPart.ToString()));
            var thirdPart = value.Substring(value.IndexOf("копій"));
            value = Convert(fractPart, 0);
            var secondPart = value.Substring(0, value.IndexOf("грив")-1);

            return new Money
            {
                Fraction = intPart,
                Cents = fractPart,
                FractionDescription = firstPart.Trim(),
                CentsDescription = secondPart.Trim(),
                CentName = thirdPart.Trim()
            };
        }

        public class Money
        {
            public long Fraction { get; internal set; }
            public long Cents { get; internal set; }
            public string FractionDescription { get; internal set; }
            public string CentsDescription { get; internal set; }
            public string CentName { get; internal set; }
        }
        ...
        public string Convert(long theMoney, long theKopeiki)
        {
            ...
            {
                money2str.Append(" " + (theKopeiki < 10 ? "0" + System.Convert.ToString(theKopeiki) : System.Convert.ToString(theKopeiki)) + " ");
            }
